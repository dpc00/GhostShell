...
| 12 | `ctypes` documentation | The three runtime ctypes files (`terminal/ghostty_vt.py`, `ai_terminal.py`'s ConPTY surface, `tools/agent_broker.py`) are fully documented (2026-09-21). The remaining undocumented ctypes is in `tools/` dev scripts (`agent_broker_client.py`, `job_breakaway_test.py`, `check_in_job.py`) and in `tools/recover_console.py`, which is a runtime file (Open in Windows Terminal), not a dev script. Owner's decision 2026-09-22: keep every file. **Open:** `recover_console.py` still needs its ctypes documented |
