...
| 9 | Logging / recording | Recorders off by default; broker log opt-in; `log_root` setting (no silent fallback) **done** 2026-09-23. `_durable_scheme_backup` unconditional, accepted (see the note in the logging section); the raw/settings debug logs (`AI_TERMINAL_DEBUG`) write only under `log_root()` (`terminal/raw_debug_log.py:17`, `terminal/log_paths.py`), so their target dir is the `log_root` setting (VERIFIED code read 2026-09-23); owner has no further decision |
