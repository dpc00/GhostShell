# ai_terminal TODO — archive (resolved/historical)


Full history of every dev-session baton for `ai_terminal`. Open/unresolved
items live in [TODO.md](TODO.md) instead — this file is append-only
...
## page_keys_to_pty audit across all AI CLI profiles (2026-08-13) — DONE

Root cause first found in Grok (earlier session, already fixed): `force_main_screen`
(`ai_terminal.sublime-settings:36`, global default `true`) strips DECSET
1049 for *every* profile, not just Grok — so any profile running a genuine
alt-screen sub-view or full-TUI reads `_tui_like()=False` regardless, and
native PageUp/PageDown steal the key from the PTY via ST's default
page-scroll instead of reaching the app. `page_keys_to_pty` (per-profile
settings override, `_page_keys_to_pty()` in `ai_terminal.py`) is the
existing escape hatch — this session was a full audit of which profiles
actually need it.

**Methodology correction, mid-session:** first pass used live-guessing
(spawn profile, try a shortcut, eyeball result) and got two profiles
wrong — Antigravity was marked "clean" from an inconclusive test when its
own shortcuts panel listed PgUp/PgDn as bound, and jcode was researched
against the wrong GitHub repo (`cnjack/jcode` — an unrelated project with
the same name; the real one is `1jehuang/jcode`, confirmed by the live
client/server codenames). Corrected approach: pull official docs/source
*before* trusting a live-test conclusion; use live DECSET-1049 detection
(monkeypatch `GhosttyParser._strip_alt_screen` via eval_python to log
every regex hit before stripping) only to confirm which state is active,
not to discover behavior from scratch. Full writeup incl. the wrong-repo
mistake in Claude memory (`project_grok_tab_scrollback_toggle.md`,
`feedback_research_before_live_testing.md`).

**`page_keys_to_pty: true` added** (confirmed via docs/source that the app
actually binds PageUp/PageDown, not just alt-screen presence):
- **Codex** + `Codex →⇢⇨ Ollama` — real alt-screen `Ctrl+T` transcript
  pager, footer literally says "pgup/pgdn to page". Verified end-to-end via
  `view.viewport_position()` staying put after the fix vs. moving before.
- **OpenCode** (+ `--mini`, `→⇢⇨ Ollama`, `→⇢⇨ Ollama --mini`) — official
  docs (`opencode.ai/docs/keybinds`) confirm `pageup`/`pagedown` bound to
  `messages_page_up`/`messages_page_down`.
- **Mimo** — confirmed fork of OpenCode (`XiaomiMiMo/MiMo-Code` README:
  "keeps all core OpenCode capabilities... TUI"), inherits the binding.
- **Qwen** (+ `→⇢⇨ Ollama`) — official docs confirm PgUp/PgDn page the
  conversation via Virtualized History (`ui.useTerminalBuffer`), on by
  default.
- **Antigravity** — its own `?` shortcuts panel explicitly lists PgUp/PgDn.
  Runs in Inline rendering mode inside `ai_terminal` (zero DECSET 1049
  observed live) so this isn't the alt-screen masking bug specifically —
  same category jcode's source proved: an inline-rendered TUI can still
  bind PageUp/PageDown for its own scroll regardless of alt-screen use.
- **jcode** — real source (`crates/jcode-tui/src/tui/app/input.rs`, main
  key handler's default match arm) unconditionally binds
  `KeyCode::Up | KeyCode::PageUp => app.scroll_up(10)` and the PageDown
  equivalent. Never enters alt-screen at all (confirmed live). Verified
  end-to-end via its `/help` overlay: PageDown moved "Help 0%" → "Help
  10%" with new content visibly scrolled in.

**Confirmed clean, no fix** (real alt-screen absent, or present but the
app doesn't bind the keys — checked, not assumed):
- **Vibe** — pre-existing profile comment (predates this audit): enters
  alt-screen but its own PageUp/PageDown do nothing internally.
- **Junie** — real alt-screen confirmed, but its own `?` help screen lists
  every bound key explicitly and PageUp/PageDown are not among them.
- **Kiro, Kimi, the plain "Claude" session tab** — zero DECSET 1049, paint
  inline, native ST page-scroll is already correct behavior for these.

**Removed from the settings/menu entirely:** Gemini CLI — confirmed live
that Google discontinued client support ("This client is no longer
supported for Gemini Code Assist for individuals... migrate to
Antigravity"), not authenticatable anymore, nothing left to keep a profile
for.

**Not audited this session:** `--chrome` sub-variants of Claude (same
binary as the audited base profile, not separately re-verified), and
`OpenCode --mini`/`OpenCode →⇢⇨ Ollama --mini` were fixed by inference
(same binary family) rather than individually live-tested.
